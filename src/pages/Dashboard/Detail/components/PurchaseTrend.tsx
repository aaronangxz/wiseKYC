...
const ProductTrend = ({ type, isSetup, description, name, icon }: IProductTrendProps) => (
  <div className={Style.productTrendPanel}>
    <Row justify='space-between'>
      ...
      <Tag theme='success'>{isSetup ? 'Activated' : 'De-activated'}</Tag>
    </Row>
...
const PurchaseTrend = () => {
  ...
  return (
    <Row className={Style.purchaseTrendPanel} gutter={[16, 16]}>
      <Col xs={12} xl={9}>
        <Card title='Trend By Account Type' subtitle='(Day)' actions={LastWeekDatePicker(onTimeChange)} headerBordered >
          <ReactEcharts
